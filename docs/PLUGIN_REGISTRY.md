...
| Miorail x402 Intelligence Seller | Agent-facing B20 / proof intelligence | `adapter` | x402 exact payment; no wallet calls returned | **Default-off** `MIORAIL_X402_SELLER_INTELLIGENCE_V1` | Free typed catalog plus three fixed-price (`0.001 USDC`, Base) resources: exact-observation B20 exit analysis, B20 liquidity evidence, and independent verification of an already owner-published Route Proof. Invalid/missing inputs fail before payment. Settlement and delivery hashes are persisted separately. No recommendation, fresh executable quote, signing, broadcasting, or calldata. Requires real settled-and-delivered production acceptance before promotion. |
