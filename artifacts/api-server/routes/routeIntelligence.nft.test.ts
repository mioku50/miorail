...
const FLAGS = {
  ...
  nftRouteV1: true,
  nftExecutionV1: true, privateAiRouteV1: false, privateAiExecutionV1: false, aerodromeExecutionV1: false,
};
...
describe('prepare loads the reviewed card, and refuses a moved listing', () => {
  ...
  test('the purchase gate being off makes nothing signable', async () => {
    nftRouteRuntime.flags = () => ({ ...FLAGS, nftExecutionV1: false, privateAiRouteV1: false, privateAiExecutionV1: false, aerodromeExecutionV1: false });
    const { prepare } = await prepared();
    assert.equal(prepare.body.signable, false);
