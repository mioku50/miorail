...
Evidence older than a day is labelled stale and describes what was true when it was checked, not what is true now.`,
  ...
  server.registerTool(
    'list_reviewed_stocks',
    {
      ...
      description:
        'Lists the reviewed underlyings Miorail has bound to Base representations, so a caller can turn a company name or ticker into the namespaced underlying key every other Stocks tool requires. Pass `query` to match a ticker, a company name or an identifier value as a case-insensitive substring; omit it to list everything. PREFER THE TICKER: most reviewed rows are named by ticker rather than by company name, so `NVDA` finds what `nvidia` does not, and a query that matches nothing says so in its own `note` rather than implying the instrument has no representation. THIS TOOL NEVER SELECTS A REPRESENTATION: it returns no contract address and no issuer choice, because different issuers publish different contracts for the same company and choosing between them is not Miorail\u2019s decision. Read `liveRepresentationCount` before comparing anything \u2014 it is a measured count of representations with tokens outstanding, and zero means nothing is outstanding on any reviewed contract at any size. An empty list means Miorail holds no reviewed binding matching that query; it is never a claim about what exists on Base or anywhere else.',
