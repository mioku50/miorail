...
describe('capability verdicts', () => {
  test('documented and callable are independent, and the verdict says which', () => {
    assert.equal(capabilityVerdictV1({ documented: 'documented', callable: 'callable' }), 'supported');
    assert.equal(
    ...
    assert.equal(capabilityVerdictV1({ documented: 'undocumented', callable: 'absent' }), 'unknown');
  });
  ...
  test('not probed is never a finding', () => {
    ...
    assert.equal(capabilityVerdictV1({ documented: 'documented', callable: 'not_probed' }), 'unknown');
...
describe('issuer profiles', () => {
  test('every issuer answers for every capability', () => {
    for (const issuer of ['coinbase', 'dinari'] as const) {
  ...
  test("Dinari's pause is supported, because the deployment answers it", () => {
    ...
    const pause = issuerCapabilitiesV1('dinari').find((state) => state.capability === 'pause_state');
  ...
  test('the B20 corporate-action surface is documented and not callable', () => {
...
describe('exit families', () => {
  test('the two are never on one axis', () => {
    ...
    assert.doesNotThrow(() => assertSameExitFamilyV1('secondary_market_cash', 'secondary_market_cash'));
  });
