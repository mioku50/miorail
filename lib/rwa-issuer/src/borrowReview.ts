...
export const BORROW_REVIEW_REFUSALS_V1 = {
  ...
  over_both_collateral_and_market:
    'Neither this wallet’s collateral nor the market’s liquidity supports this amount. Both are short, so adding collateral alone would not make it available.',
  nothing_to_borrow:
  ...
  venue_would_not_prepare:
    'The venue did not prepare this borrow, so there is nothing to review. That is a fact about the venue’s answer, not about this wallet or this market.',
  prepared_calls_refused:
    'Miorail refused the calls the venue prepared, before any of them reached a wallet. What was wrong with them is recorded beside this review.',
  venue_and_miorail_disagree:
    'The venue’s own simulation and Miorail’s disagree about whether these calls execute. Until they agree neither answer is being relied on, and nothing is offered for approval.',
  // The four the simulation produces. A review may not reach
  // `ready_for_your_approval` without a measured execution behind it, and no
  // two of these may share a sentence: they are opposite facts.
  simulation_not_run:
  ...
  simulation_effect_unread:
    'The calls execute, and Miorail could not establish what reaches this wallet. That is a gap in Miorail’s reading rather than a finding about the transaction — and it is not a pass.',
  simulation_delivered_something_else:
    'The calls execute, what reaches this wallet was read, and it is not what this review describes. Nothing is being offered for approval on that basis.',
} as const;
