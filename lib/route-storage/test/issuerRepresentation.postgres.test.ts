...
import { createDatabaseIssuerRepresentationRepository } from '../src/issuerRepresentationDatabase.js';
...
//     npx tsx --test lib/route-storage/test/issuerRepresentation.postgres.test.ts
//
...
if (!throwaway) {
  describe('postgres issuer representation', () => {
    test('skipped: set MIOAGENT_MIGRATION_TEST_URL to a throwaway local database', () => {});
  });
} else {
  issuerRepresentationContractV1('postgres', async () => {
    await sql!.unsafe('TRUNCATE issuer_representation, issuer_membership_check');
    const executor: SqlTemplateExecutor = (strings, ...values) =>
      (sql as unknown as (
        strings: TemplateStringsArray,
        ...values: unknown[]
      ) => Promise<Record<string, unknown>[]>)(strings, ...values);
    return { repository: createDatabaseIssuerRepresentationRepository(executor) };
  });
}
