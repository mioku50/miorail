...
export interface StocksConsoleInputV1 {
  ...
  /**
   * Growth plan step 2 — Buy and Sell on the answer card, each at an amount the
   * reader types, through the same `onPrepare` handoff as every other trade.
   * Absent: the card keeps its Prepare buttons. The Base App leaves it out
   * because it has no sign-in to carry a trade through.
 ...
 * is decided per approval, and a spent allowance falls back to the wallet
 * paying. */
export function sponsoredFeeNoteV1(input: { sponsoredGas: boolean; dailyLimitPerWallet: number | null }): string | null {
...
export function useStocksConsoleV1(input: StocksConsoleInputV1): StocksConsoleResultV1 {
  ...
  const model: MarketRealityScreenModelV1 = {
    ...
    trade:
      input.trade && input.onPrepare && reality.data
        ? {
            note: sponsoredFeeNoteV1(input.trade),
    ...
    actions: {
      ...
      ...(reality.data && input.onInspectRoute
        ...
        : {}),
      // Growth plan step 2 — the same handoff at the reader's own amount. Every
      // check it makes (a reviewed address, supply outstanding, a reviewed
      // router policy) still applies; only the size is the reader's, and the
      // route is planned, quoted and simulated again at that size before
      // anything is signed. A refusal comes back as a sentence for the card
      // rather than a button that silently does nothing.
      ...(reality.data && input.onPrepare && input.trade
        ? {
            onTrade: (trade: { tokenAddress: string; direction: 'buy' | 'sell'; amountAtomic: string }) => {
              const built = stockTradeGoalV1({ ...trade, response: reality.data as never, now: new Date() });
