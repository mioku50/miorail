...
> [!NOTE]
> Vendored from base/skills. The shared API key printed in the upstream document is intentionally REDACTED here: this repo only ever reads the key from the server-side `UNISWAP_API_KEY` environment variable and never commits it.
...
```json
{
  ...
  "x-api-key": "<UNISWAP_API_KEY from server-side env — never hardcoded>"
}
```
